...
if(document.querySelector("#summary")){
 ...
 if(!o) location.href="index.html";
  ...
  const likesPer=Math.floor(Number(p.likes.replaceAll(".",""))/p.posts).toLocaleString("pt-BR");
  const viewsPer=Math.floor(Number(p.views.replaceAll(".",""))/p.posts).toLocaleString("pt-BR");
  document.querySelector("#summary").innerHTML=`<small>PLANO SELECIONADO</small><h2>🚀 ${p.name}</h2><p>${o.service}</p><hr><div class="plan-details"><div class="detail"><b>👤 ${p.followers}</b><span>seguidores</span></div><div class="detail"><b>❤️ ${p.likes}</b><span>curtidas</span></div><div class="detail"><b>👁️ ${p.views}</b><span>visualizações</span></div><div class="detail"><b>📸 ${p.posts}</b><span>publicações</span></div>${p.comments?`<div class="detail"><b>💬 ${p.comments}</b><span>comentários</span></div>`:""}</div><div class="per-post">📲 Por publicação: <b>${likesPer} curtidas</b> + <b>${viewsPer} visualizações</b>${p.comments?` + <b>${p.comments} comentários</b>`:""}</div><div class="total"><span>Valor do plano</span><strong>${p.price}</strong></div>`;
